...
public class EncryptAlgorithms {
	...
	// Caesar-Algorithmus;
	// Ver- oder entschlüsselt einen String mit einem integer nach dem
	// Caesar-Verfahren
	public static String cesarEncrypt(String startText, int key, boolean encrypt) {
	...
	// Ver- oder entschlüsselt einen String mit einem Schlüsselstring nach dem
	// Vigenere-Verfahren
	public static String vigenereEncrypt(String startText, String key, boolean encrypt) {
		...
		// wandle den (Buchstaben-)Schlüsselarray zu einem nutzbaren Int-Array
		// um
	...
	public static char crypt(char startC, int key) {
		// Variablendeklaration:
		int pInt = startC;
		int endInt = 0;

		// Führe jeweils die shift- Funktion aus
		if ((64 < pInt) && (pInt < 91)) { // wenn im Bereich der Großbuchstaben,
											// ziehe den Wert von "A" ab
			pInt -= 65;
			endInt = shift(pInt, key) + 65;
		} else if ((96 < pInt) && (pInt < 123)) { // wenn im Bereich der
													// Kleinbuchstaben, ziehe
													// den Wert von "a" ab
			pInt -= 97;
			endInt = shift(pInt, key) + 97;
		} else {
			return startC; // wenn weder Groß- noch Kleinbuchstabe, gib den
							// Buchstaben direkt zurück
		}
		return (char) endInt; // gib den zu einem Buchstaben verwandelten Int
								// zurück
	}

	// verschiebt einen Integer pInt um key Anzahl an Stellen
	public static int shift(int pInt, int key) {
		int cInt = 0;

		cInt = pInt + key;

		if (cInt < -26) { // cInt auf einen Bereich zwischen -26 und 0 legen,
							// falls niedriger
			cInt %= 26;
		}
		if (cInt < 0) { // falls cInt < 0, addiere 26. Dadurch wird ein Wert
						// zwischen 0 und 26 erzeugt
			cInt += 26;
		}
